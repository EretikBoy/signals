...
class DataManager:
    ...
    def update_analysis_params(self, subject_code, analysis_index, new_params):
        """Обновление параметров анализа"""
        if subject_code in self.subjects_data and analysis_index in self.subjects_data[subject_code]:
            # Обновляем параметры
            self.subjects_data[subject_code][analysis_index]['params'].update(new_params)

            # Обновляем параметры в процессоре, если он существует
            processor = self.subjects_data[subject_code][analysis_index].get('processor')
            if processor:
                processor.update_params(new_params)
