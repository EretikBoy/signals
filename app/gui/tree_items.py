...
class AnalysisItem(QTreeWidgetItem):
    ...
    def __init__(self, subject_code, analysis_index, file_data, processor=None):
        super().__init__()
        self.subject_code = subject_code
        self.analysis_index = analysis_index
        self.file_data = file_data
        self.processor = processor  # Сохраняем ссылку на процессор
    ...
    def setup_display(self):
        ...
        # Параметры будут заполняться динамически через TreeManager
    ...
    def update_dynamic_columns(self, column_values):
        """Обновление динамических столбцов"""
        for col_index, value in enumerate(column_values):
            self.setText(4 + col_index, value)
...
class SubjectItem(QTreeWidgetItem):
    ...
    def add_analysis(self, file_data, analysis_index=None, processor=None):
        ...
        analysis_item = AnalysisItem(self.subject_code, analysis_index, file_data, processor)
